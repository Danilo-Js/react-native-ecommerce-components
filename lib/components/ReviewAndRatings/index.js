...
const ReviewAndRatings = ({ reviews, averageRating, totalReviews, onAddReview, title, subTitle, language = 'en', styles: customStyles = {}, }) => {
    ...
    const renderReview = ({ item }) => (<View style={[styles.reviewContainer, customStyles.reviewContainer]}>
    ...
    return (<View style={[styles.container, customStyles.container]}>
      ...
      <FlatList data={reviews} renderItem={renderReview} keyExtractor={(item) => item.id} style={{ marginBottom: hp('2%') }}/>

      <View style={[styles.inputContainer, customStyles.inputContainer]}>
        <TextInput style={[styles.input, customStyles.input]} placeholder={t.placeholderName} value={user} onChangeText={setUser}/>
        <TextInput style={[styles.input, customStyles.input]} placeholder={t.placeholderComment} value={comment} onChangeText={setComment} multiline/>
        <TextInput style={[styles.input, customStyles.input]} placeholder={t.placeholderRating} value={rating !== null ? rating.toString() : ''} onChangeText={(v) => setRating(Number(v) || null)} keyboardType="numeric"/>
